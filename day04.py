...
print(sum(1 for x in puzzle_input if check_if_valid(x)))

def check_if_valid_anagram(line):
    words = set()
    for el in line.split():
        if ''.join(sorted(el)) not in words:
            words.add(''.join(sorted(el)))
        else:
            return False
    return True

print(sum(1 for x in puzzle_input if check_if_valid_anagram(x)))